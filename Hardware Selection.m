As of March 2026, there are great deals on ebay for old Dell Optiplex, HP and Lenovo systems.

Top choices include:
Elitedesk G4 800 (8500 or above)
Elitedesk G5 800 (9500 or above)
Dell Optiplex 5060 (8500 or above)
Dell Optiplex 7070 (9500 or above)
Lenovo 920s (8500 or above)

The cheapest units are no SSD and no OS units. 
Assuming you have an old SSD / m.2 drive lying around, these are fantastic choices that you can grab for 130-160 USD.

These are great choices as they have 6 cores, an iGPU, and will run well in headless mode.
They are also optimized to run in dusty office environments with minimal care.

Preparation steps should include:  
1. Download the manual and familiarize yourself with how to open and care for the machine  
2. Open up the unit and clean out the dust  
3. Install SSD or m.2 as per manual  
4. Update BIOS to newest version by connecting to hp.com / dell.com / lenovo.com (must use ethernet)  
5. If upgrading RAM, install ram kit  
6. Check RAM stability. Download memtest86+, run from bootable USB. Execute 2 test passes minimum, ideally 4.
